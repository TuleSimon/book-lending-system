...
import com.abby.booklendingsystem.databinding.FragmentMySalesBinding
...
@AndroidEntryPoint
class MySalesFragment() : Fragment(),ViewBookClick{


    lateinit var binding:FragmentMySalesBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentMySalesBinding.inflate(inflater,container,false)
        initRecyclerView()
        return binding.root
    }
    ...
    private fun getNewBooks(){
        lifecycleScope.launch {
            FirebaseAuth.getInstance().currentUser?.uid?.let { pbViewModel.getMySales(it) }
        }
    }

...
import com.abby.booklendingsystem.databinding.FragmentNewBooksBinding
...
@AndroidEntryPoint
class UsedBooksFragment(var type:String) : Fragment(), ViewBookClick{


    lateinit var binding:FragmentNewBooksBinding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        booksViewModel.usedBooks.observe(viewLifecycleOwner){
                ...
                is NetworkResult.Success -> {
                    ...
                    data?.apply {
                        ...
                        if(this.isEmpty()){
                            ...
                            binding.errorlayout.textView.text = "No Books available Yet"
                            binding.errorlayout.retryButton.setOnClickListener {
                                getNewBooks()
                            }
                        }
    ...
    private fun getNewBooks(){
        lifecycleScope.launch {
            ...
            {
                FirebaseAuth.getInstance().currentUser?.let { booksViewModel.getUsedBooks(it.uid) }
            }
            else
                booksViewModel.getUsedBooks()
        }
    }
    ...
    lateinit var list:List<BookModel>
    private fun setFilter(){


        binding.filterCategory2.setOnItemSelectedListener { _, _, id, item ->
        ...
        binding.searchGo.setOnClickListener {
            val searchText = binding.searchBooksText2.text.toString()
    ...
    private fun initializeNetworkDependentValues() = lifecycleScope.launch{
        ...
        homeViewModel.categoriesResult.observe(viewLifecycleOwner){
            when(it){
                is NetworkResult.Error -> {
                    Utils.dismissLoader()
                    binding.filterCategory2.isEnabled = false
                    Toasty.error(
                        requireContext(),
